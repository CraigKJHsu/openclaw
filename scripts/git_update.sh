...
if [ -z "$(git config --global user.email)" ] || [ -z "$(git config --global user.name)" ]; then
    ...
    # 設置默認用戶身份
    git config --global user.email "kj@ingrids.app"
...
show_push_failure_help() {
    ...
    echo "      git push origin grid_trading_bot_with_UI --force"
...
# 檢查是否有本地提交需要推送
if [ -n "$(git log origin/grid_trading_bot_with_UI..HEAD 2>/dev/null)" ]; then
    # 推送允許失敗（因為可能是認證問題，不應阻止腳本完成）
    PUSH_CMD="git push origin grid_trading_bot_with_UI --force"
    
    # 優先使用 GITHUB_TOKEN 環境變數（如果存在）
    if [ -n "$GITHUB_TOKEN" ]; then
        echo "正在推送到遠程倉庫（使用 GITHUB_TOKEN 環境變數）..."
        # 使用 Token 在 URL 中進行認證
        git remote set-url origin "https://${GITHUB_TOKEN}@github.com/CraigKJHsu/grid_trading_bot.git" 2>/dev/null || true
        if ! run_git_command "正在推送到遠程倉庫..." "true" $PUSH_CMD; then
            # 如果失敗，恢復原始 URL
            git remote set-url origin "https://github.com/CraigKJHsu/grid_trading_bot.git" 2>/dev/null || true
            show_push_failure_help
        else
